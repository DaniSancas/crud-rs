...
impl Api {
    ...
    async fn get_article(&self, id: Path<ArticleId>, data: Data<&SqlitePool>) -> GetArticleResponse {
    ...
    /// Create Article
    #[oai(path = "/article/create", method = "post")]
    async fn create_article(&self, article: Json<Article>, data: Data<&SqlitePool>) -> CreateArticleResponse {
        let article_repository = SqliteArticleRepository::new(data.0.clone());
        let article_use_case = ArticleUseCase::new(article_repository);
